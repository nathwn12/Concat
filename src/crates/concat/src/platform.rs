...
fn unpack_monitor(bits: u64) -> Option<(u32, u32)> {
    (bits != 0).then_some(((bits >> 32) as u32, bits as u32))
}

/// The window's raw events, seen before Slint routes them. Three jobs:
///
...
/// the same shape a picked-files dialog hands the caller, so the caller
/// need not know drag and drop split it up.
///
...
///
/// The window's monitor. It is the only place the monitor is seen: the
/// launch form is published before `App::run` maps a window, so
/// `monitor_size` has nothing to read then, and this handler - handed the
/// winit window on its every event - is where the real measurement comes
/// from. `on_monitor` fires when that measurement changes, so the caller
/// can re-publish.
#[cfg(not(target_os = "android"))]
struct DropHandler {
    pending: Vec<PathBuf>,
